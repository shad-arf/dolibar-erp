...
var $FontType;           //indicates whether font is truetypeunicode or not
...
function __construct($orientation='P',$unit='mm',$format='A4')
{
	...
	if(is_string($format))
	{
		...
		if($format=='a1')
			$format=array(1685,2384);
		elseif($format=='a2')
			$format=array(1190,1684);
		elseif($format=='a3')
		...
		elseif($format=='us-letter')
			$format=array(612,792);
		elseif($format=='us-legal')
			$format=array(612,1008);
		elseif($format=='us-ledger')
			$format=array(1224,792);
		else
...
//cambio 2byte.es fin
	...
	switch($dest)
	{
		...
		case 'S':
			...
			{
					//$this->Error('Some data has already been output to browser, can\'t send PDF file, file: '.$archivo.', line '.$linea);
			}
			return $this->buffer;
		default:
...
function _putpages()
{
	...
	for($n=1;$n<=$nb;$n++)
	{
		...
		//cambio 2byte.es		
		if ($db->forcecharset == 'latin1'){
			$this->pages[$n] = utf8_decode($this->pages[$n]);
		}
