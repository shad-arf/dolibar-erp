...
class reportico_report_html extends reportico_report
{
		...
		if ( $this->line_count < 1 )
		{
		    ...
		    if ( !get_request_item("printable_html") )
            {
		        ...
		        if ( get_reportico_session_param("show_refresh_button") )
			        $this->text .= '<div class="swRepRefreshBox"><a class="swLinkMenu" href="'.$this->query->get_action_url().'?'.$forward.'refreshReport=1&execute_mode=EXECUTE&reportico_session_name='.reportico_session_name().(version_compare(DOL_VERSION, 13) >= 0? "&token=".newToken():'').'" title="'.template_xlate("GO_REFRESH").'">&nbsp;</a></div>';
		        $this->text .= '<div class="reporticoJSONExecute"><a class="swJSONExecute1 testy" href="'.$this->query->get_action_url().'?'.$forward.'refreshReport=1&target_format=JSON&execute_mode=EXECUTE&reportico_session_name='.reportico_session_name().(version_compare(DOL_VERSION, 13) >= 0? "&token=".newToken():'').'" title="'.template_xlate("GO_REFRESH").'">&nbsp;</a></div>';
            }
            else
            {
		        $this->text .= '<div class="swRepPrintBox"><a class="swLinkMenu" href="'.$this->query->get_action_url().'?'.$forward.'printReport=1&execute_mode=EXECUTE&reportico_session_name='.reportico_session_name().(version_compare(DOL_VERSION, 13) >= 0? "&token=".newToken():'').'" title="'.template_xlate("GO_PRINT").'">'.template_xlate("GO_PRINT").'</a></div>';
            }
	...
	function abs_paging_height($height_string)
	{
		...
		//if ( strstr($height_string, "%" )  2byte.es problems with php 7.1
	...
	function format_column(& $column_item)
	{
		...
		if (function_exists('mb_check_encoding')) {
			if (!mb_check_encoding($padstring, "UTF-8")) {
				$padstring = mb_convert_encoding($padstring, "UTF-8", "HTML-ENTITIES");
			}
		}
		else {
    ...
    function format_hyperlinks ( $hyperlinks, $padstring )
    {
		...
		//cambio 2byte.es
		$url = dol_buildpath($hyperlinks["url"],1);

        // Add any application specific url params
		//2byte.es ini
        if (version_compare(DOL_VERSION, 6.0) >= 0) {
            $url = str_replace("soc.php", "card.php", $url);
            $url = str_replace("facture.php", "facture/card.php", $url);
            //moviments bancaris
            //$url = str_replace("dolibarr/comm/card.php", "comm/card.php", $url);
        }
        else{
            $url = str_replace("card.php","soc.php", $url);
            $url = str_replace("facture/card.php", "facture.php", $url);
...
//echo "<BR>";
	...
	function format_headers()
	{
		...
		if ( $this->body_display != "show" )
			return;
		//cambio 2byte.es
		$this->text .="<thead><tr class='liste_titre'>";
	...
	function begin_page()
	{
	    ...
	    if ( !get_request_item("printable_html") )
        {
            if ( !$this->query->access_mode || ( $this->query->access_mode != "REPORTOUTPUT" )  )
            {
			    $this->text .= '<div class="swRepBackBox"><a class="swLinkMenu" href="'.$this->query->get_action_url().'?'.$forward.'execute_mode=PREPARE&reportico_session_name='.reportico_session_name().(version_compare(DOL_VERSION, 13) >= 0? "&token=".newToken():'').'" title="'.template_xlate("GO_BACK").'">&nbsp;</a></div>';
            }
	        if ( get_reportico_session_param("show_refresh_button") )
		        $this->text .= '<div class="swRepRefreshBox"><a class="swLinkMenu" href="'.$this->query->get_action_url().'?'.$forward.'refreshReport=1&execute_mode=EXECUTE&reportico_session_name='.reportico_session_name().(version_compare(DOL_VERSION, 13) >= 0? "&token=".newToken():'').'" title="'.template_xlate("GO_REFRESH").'">&nbsp;</a></div>';
        }
        ...
        {
	        $this->text .= '<div class="swRepPrintBox"><a class="swLinkMenu" href="'.$this->query->get_action_url().'?'.$forward.'printReport=1&execute_mode=EXECUTE&reportico_session_name='.reportico_session_name().(version_compare(DOL_VERSION, 13) >= 0? "&token=".newToken():'').'" title="'.template_xlate("GO_PRINT").'">'.'&nbsp;'.'</a></div>';
        }
	}
