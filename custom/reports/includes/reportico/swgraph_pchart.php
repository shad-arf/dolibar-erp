...
class reportico_graph
{
	...
	if ( !$forward_url_params )
        ...
        $url .= "&reportico_session_name=".reportico_session_name().(version_compare(DOL_VERSION, 13) >= 0? "&token=".newToken():'');
...
$graphData->setXAxisName(sw_translate($this->xtitle));
...
function setSerieDrawable($plot, $graphData, $inseries,$flag)
{
    foreach ( $plot as $k => $v )
    {
	    ...
	    if ( $inseries == $series )
        {
            ...
            {
	            $graphData->AddSerie($series);
	            $graphData->SetSerieName(sw_translate($v["legend"]), $series);
            }
            else
	            $graphData->RemoveSerie($series);
        }
