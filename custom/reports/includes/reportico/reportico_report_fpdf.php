...
class reportico_report_fpdf extends reportico_report
{
	...
	function finish ()
	{
		...
		else
		{
			...
			print($buf);
			die;*/
			$file_temp = dol_buildpath("/reports/includes/reportico/templates_c/",0).dol_now().".pdf";
	...
	function format_column_header(& $column_item)   //PDF column headers
	{
		...
		//cambio 2byte.es$padstring = ucwords(strtolower($padstring));
	...
	function plot_graph(&$graph, $graph_ct = false)
	{
        ...
        {
			//cambio 2byte.es
		    $handle = $graph->generate_graph_image($tmpnam.".png");
		    unlink($tmpnam);
	...
	function format_page_header(&$header)
	{
		...
		//cambio 2byte.es
        $tx = $this->reportico_string_to_php(sw_translate($tx));
		$tx = reportico_assignment::reportico_meta_sql_criteria($this->query, $tx);
        $tx = preg_replace("/<\/*u>/", "", $tx);
