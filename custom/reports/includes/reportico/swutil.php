...
function &sw_translate($in_string)
{
	...
	if (! empty($out_string))
		$out_string = $langs->transnoentities($out_string);
	/*if ( $g_translations )
...
function &sw_translate_report_desc($in_report)
{
	...
	$out_string = false;
	$out_string = $langs->trans($in_report."_desc");
	/*if ( $g_report_desc )
...
{
        /*cambio 2byte.es ini$newpath = "reportico.php";
        $found = find_file_to_include($newpath, $newpath, $reltoinclude);
        $newpath = get_relative_path(str_replace ("/", "\\", realpath($newpath)), dirname(realpath($_SERVER["SCRIPT_FILENAME"])));
        ...
        if ( $url_path == "/" || $url_path == "\\" )
            ...
            $url_path = $url_path . "/";*/
		$url_path = dol_buildpath("reports/includes/reportico/",1);
		//cambio 2byte.es fin
