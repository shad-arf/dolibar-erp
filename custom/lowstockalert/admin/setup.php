...
/**
 * \file    lowstockalert/admin/setup.php
 * \ingroup lowstockalert
 * \brief   LowStockAlert setup page.
 */
...
require_once '../lib/lowstockalert.lib.php';
//require_once "../class/myclass.class.php";

// Translations
$langs->loadLangs(array("admin", "lowstockalert@lowstockalert"));
...
$arrayofparameters = array(
    'LOWSTOCKALERT_EMAILOBJECT'=>array('type'=>'string', 'css'=>'minwidth500' ,'enabled'=>1),
    'LOWSTOCKALERT_EMAILFROM'=>array('type'=>'string','css'=>'minwidth500' ,'enabled'=>1),
    'LOWSTOCKALERT_EMAILTO'=>array('type'=>'string','css'=>'minwidth500' ,'enabled'=>1),
    'LOWSTOCKALERT_EMAILMESSAGE'=>array('type'=>'textarea', 'enabled'=>1),
//    'LOWSTOCKALERT_EMAILMESSAGE'=>array('type'=>'html', 'enabled'=>1),
    'LOWSTOCKALERT_ALERT'=>array('type'=>'yesno', 'enabled'=>1),
);
...
if ($action == 'updateMask') {
	...
	foreach ($dirmodels as $reldir) {
		$file = dol_buildpath($reldir."core/modules/lowstockalert/doc/pdf_".$modele."_".strtolower($tmpobjectkey).".modules.php", 0);
		if (file_exists($file)) {
			$filefound = 1;
	...
	if (!empty($tmpobjectkey)) {
		$constforval = 'LOWSTOCKALERT_'.strtoupper($tmpobjectkey)."_ADDON";
	...
	if ($ret > 0) {
		$tmpobjectkey = GETPOST('object');
		if (!empty($tmpobjectkey)) {
			$constforval = 'LOWSTOCKALERT_'.strtoupper($tmpobjectkey).'_ADDON_PDF';
...
$head = lowstockalertAdminPrepareHead();
print dol_get_fiche_head($head, 'settings', $langs->trans($page_name), -1, "lowstockalert@lowstockalert");

// Setup page goes here
echo '<span class="opacitymedium">'.$langs->trans("LowStockAlertSetupPage").'</span><br><br>';
					...
					while (($file = readdir($handle)) !== false) {
						if (strpos($file, 'mod_'.strtolower($myTmpObjectKey).'_') === 0 && substr($file, dol_strlen($file) - 3, 3) == 'php') {
							...
							if ($module->isEnabled()) {
								...
								$constforvar = 'LOWSTOCKALERT_'.strtoupper($myTmpObjectKey).'_ADDON';
