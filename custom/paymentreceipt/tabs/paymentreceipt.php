...
dol_include_once("paymentreceipt/paymentreceipt_consts.php");
dol_include_once("paymentreceipt/core/modules/paymentreceipt/modules_paymentreceipt.php");
...
if ($action == 'builddoc') { // En get ou en post
    ...
    $result = paiement_pdf_create($db, $object, '', $object->modelpdf ? $object->modelpdf : 'orge', $langs, GETPOST('hidedetails'), GETPOST('hidedesc'), GETPOST('hideref'));
    if ($result <= 0) {
...
// Remove file in doc form
    ...
    if ($object->fetch($id)) {
        ...
        $langs->load("other");
        $upload_dir = $conf->paymentreceipt->dir_output;
...
dol_fiche_head($head, 'paymentreceipt', $langs->trans("PaymentCustomerInvoice"), 0, 'payment');
...
$paiement->fetch($id);
$filename = dol_sanitizeFileName($paiement->ref);

// Definition of $dir and $file
$filedir = DOL_DATA_ROOT . '/paymentreceipt/' . dol_sanitizeFileName($paiement->ref);
$urlsource = $_SERVER['PHP_SELF'] . '?id=' . $id;
$genallowed = $user->rights->facture->creer;
$delallowed = $user->rights->facture->supprimer;

print '<br>';
print showdocuments($db, 'paymentreceipt', $filename, $filedir, $urlsource, $genallowed, $delallowed, 'orge');
