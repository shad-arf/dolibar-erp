 ...
 *	\ingroup    commande
 *	\brief      File of Class to generate PDF orders from pdf_ultimate_order model 
 */
...
/**
 *	Class for generate the commands with ultimate_order template
 */
class pdf_ultimate_order extends ModelePDFCommandes
{
	...
	public function write_file($object, $outputlangs, $srctemplatepath = '', $hidedetails = 0, $hidedesc = 0, $hideref = 0)
	{
		...
		if ($conf->commande->multidir_output[$conf->entity]) {
			...
			if ($object->specimen) {
				...
				if ($conf->mbisignature->enabled) {
					...
					if ($obj->pathoffile !== 'document generated' && $obj->pathoffile) {
						...
						/////// MB INFORMATIQUE
						$dir = $conf->commande->multidir_output[$conf->entity] . "/" . $objectref;
			...
			if (file_exists($dir)) {
				...
				$tab_top_newpage = (empty($conf->global->ULTIMATE_ORDERS_PDF_DONOTREPEAT_HEAD) ? $this->marge_haute + $logo_height + 10 : 10);
				...
				if ($conf->incoterm->enabled) {
					...
					if ($desc_incoterms) {
						...
						$pdf->RoundedRect($this->marge_gauche, $tab_top, $tab_width, $height_incoterms + 1, 2, '1111', 'S', $this->border_style, $bgcolor);
						$tab_top = $nexY + 10;
					}
				}
				...
				if ($notetoshow && empty($conf->global->MAIN_PUBLIC_NOTE_IN_ADDRESS)) {
					...
					if ($pageposafternote > $pageposbeforenote) {
						...
						while ($pagenb < $pageposafternote) {
							...
							if (empty($conf->global->ULTIMATE_ORDERS_PDF_DONOTREPEAT_HEAD)) {
				...
				for ($i = 0; $i < $nblines; $i++) {
					...
					if ($this->getColumnStatus('desc')) {
						...
						if (!empty($conf->global->ULTIMATE_PRODUCT_ENABLE_CUSTOMCOUNTRYCODE) && $object->lines[$i]->product_type != 9 && $object->lines[$i]->product_type != 1) {
							...
							$posYAfterCustom = $posYAfterDescription + 4;
						}
					...
					// Add line
					if (!empty($conf->global->ULTIMATE_ORDER_PDF_DASH_BETWEEN_LINES) && $i < ($nblines - 1) && $object->lines[$i]->product_type != 9 && $object->lines[$i + 1]->product_type != 9 && !($object->lines[$i + 1]->pagebreak == true)) {
						$pdf->setPage($pageposafter);
						$pdf->SetLineStyle(array('dash' => '1, 1', 'color' => array(70, 70, 70)));
						if ($conf->global->ULTIMATEPDF_GENERATE_ORDERS_WITH_PRODUCTS_BARCODE == 1 || !empty($conf->global->ULTIMATE_PRODUCT_ENABLE_CUSTOMCOUNTRYCODE)) {
	...
	function _tableau_tot(&$pdf, $object, $deja_regle, $posy, $outputlangs)
	{
		...
		if ($conf->global->ULTIMATE_GENERATE_ORDERS_WITH_WEIGHT_COLUMN == 1 && $conf->global->ULTIMATE_GENERATE_ORDERS_WITH_TOTAL_WEIGHT == 1) {
			//Total Weigth
	...
	protected function _agreement(&$pdf, $object, $posy, $outputlangs)
	{
		...
		if (!empty($conf->global->ULTIMATE_DISPLAY_ORDER_AGREEMENT_BLOCK)) {
			...
			$deltay = $this->page_hauteur - $heightforfreetext - $heightforfooter - $heightforagreement / 2 + 5;
			$posy = $deltay;
			$deltax = $this->marge_gauche + $widthrecbox + 4;
			if ($roundradius == 0) {
				$roundradius = 0.1; //RoundedRect don't support $roundradius to be 0
	...
	protected function _pagehead(&$pdf, $object, $showaddress, $outputlangs, $titlekey = "PdfOrderTitle")
	{
		...
		// Translations
		$outputlangs->loadLangs(array("main", "dict", "companies", "bills", "propal", "orders", "deliveries", "commercial", "projects", "ultimatepdf@ultimatepdf"));
		...
		if (!empty($conf->global->ULTIMATEPDF_GENERATE_ORDERS_WITH_TOP_QRCODE) || ($conf->global->ULTIMATEPDF_GENERATE_ORDERS_WITH_MYCOMP_QRCODE)) {
		...
		$pdf->SetXY($posx, $posy);
		$pdf->SetTextColorArray($textcolor);
		$pdf->MultiCell($rightSideWidth, 4, $outputlangs->transnoentities("OrderDate") . " : " . dol_print_date($object->date, "%d %b %Y", false, $outputlangs, true), '', 'R');
		...
		if (!empty($conf->global->ULTIMATE_ORDERS_PDF_SHOW_PROJECT)) {
			...
			if (!empty($object->project->ref)) {
				...
				$pdf->SetFont('', '', $default_font_size - 1);
				$pdf->SetXY($posx, $posy);
				$pdf->MultiCell($rightSideWidth, 4, $outputlangs->transnoentities("RefProject") . " : " . (empty($object->project->ref) ? '' : $object->projet->ref), 0, 'R');
			}
		}
		...
		if ($showaddress) {
			// Sender properties
			$carac_emetteur = '';
			// Add internal contact of order if defined
			...
			if ($object->getIdContact('external', 'BILLING') && $object->getIdContact('external', 'SHIPPING')) {
				...
				if ($conf->global->ULTIMATE_CONTACT_ADDRESS_BLOCKS_OVER == 1) {
					...
					$mode = 'target';
					$carac_client = pdf_element_build_address($outputlangs, $this->emetteur, $object->thirdparty, ($usecontact ? $object->contact : ''), $usecontact, $mode, $object, false);
			...
			elseif ($object->getIdContact('external', 'CUSTOMER') && $object->getIdContact('external', 'SHIPPING')) {
				...
				if ($conf->global->ULTIMATE_CONTACT_ADDRESS_BLOCKS_OVER == 1) {
					...
					$mode = 'target';
					$carac_client = pdf_element_build_address($outputlangs, $this->emetteur, $thirdparty, $object->contact, $usecontact, $mode, $object);
			...
			elseif ($object->getIdContact('external', 'CUSTOMER') && $object->getIdContact('external', 'BILLING')) {
				...
				// Recipient address
				$mode = 'target';
				$carac_client = pdf_element_build_address($outputlangs, $this->emetteur, $thirdparty, $object->contact, $usecontact, $mode, $object, true);
				...
				if ($conf->global->ULTIMATE_CONTACT_ADDRESS_BLOCKS_OVER == 1) {
					...
					// Show commande_external_CUSTOMER address
					$pdf->SetAlpha($opacity);
					$pdf->RoundedRect($posx, $posy, $widthrecboxrecipient, $hautcadre * 0.5, $roundradius, $round_corner = '1111', $receiptstyle, $this->border_style, $bgcolor);
					$pdf->SetAlpha(1);
					$pdf->SetXY($posx + 2, $posy - 0.5);
	...
	protected function _pagefoot(&$pdf, $object, $outputlangs, $hidefreetext = 0)
	{
		...
		return pdf_ultimatepagefoot($pdf, $outputlangs, 'ORDER_FREE_TEXT', $this->emetteur, $this->marge_basse, $this->marge_gauche, $this->page_hauteur, $object, $showdetails, $hidefreetext, $this->footertextcolor);
	}
	...
	public function defineColumnField($object, $outputlangs, $hidedetails = 0, $hidedesc = 0, $hideref = 0)
	{
		...
		if (!empty($conf->global->ULTIMATE_ORDERS_WITH_LINE_NUMBER) && $conf->global->ULTIMATE_PDF_BORDER_LEFT_STATUS == 'yes') {
		...
		if (!empty($conf->global->ULTIMATE_ORDERS_WITH_LINE_NUMBER &&$conf->global->ULTIMATE_PDF_BORDER_LEFT_STATUS == 'yes') || ($conf->global->ULTIMATE_DOCUMENTS_WITH_REF == "yes" && $this->atleastoneref == true &&$conf->global->ULTIMATE_PDF_BORDER_LEFT_STATUS == 'yes')) {
